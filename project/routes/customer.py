...
def new_loan(loggedInUser):
    ...
    return jsonify({'Message': 'Your Loan Application has
                    been successfully created', "data":
                    {
                        ...
                        "EMI": new_loan.emi,
                        "Total amount to
                        be paid": new_loan.total_payable_amount,
...
def register_Customer():
    """Endpoint to register/add more users to the loan system"""
    data = request.get_json()
    """Creating new User."""
    ...
    db.session.commit()
    return jsonify({'Message': 'Your Account has
